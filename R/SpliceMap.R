...
SpliceMap <- function(cfg) {
    ...
    ret <- system2(file.path(system.file(package = "Rbowtie"), "runSpliceMap_QuasR"),
                   callstr, stdout = TRUE, stderr = FALSE)
    ...
    ret <- system2(file.path(system.file(package = "Rbowtie"),"bowtie"),
                   callstr, stdout = TRUE, stderr = TRUE)
    ...
    ret <- system2(file.path(system.file(package = "Rbowtie"), "sortsam"),
                   callstr, stdout = TRUE, stderr = TRUE)
    ...
    refL <- scan(file.path(cfg[['temp_path']], "ref_list"),
                 what = list(chrName = "", chrFile = "", chrDir = "",
                             chrS = "", chrE = ""),
                 sep = "\t", quiet = TRUE)
    ...
    if (!is.na(suppressWarnings(packageDescription("parallel", fields = "Version")))) {
        ...
        ret <- parallel::parLapplyLB(cl, callstr, system2,
                                     command = file.path(system.file(package = "Rbowtie"), "SpliceMap"),
                                     stdout = TRUE, stderr = FALSE)
        parallel::stopCluster(cl)
    } else {
        message("[SpliceMap] finding spliced alignments for ",
                length(refL[['chrName']]), " chromosomes using serial processes...", appendLF = FALSE)
        ret <- lapply(callstr, system2,
                      command = file.path(system.file(package = "Rbowtie"), "SpliceMap"),
                      stdout = TRUE, stderr = FALSE)
    }
    if (length(ret) == length(callstr) && all(lengths(ret) > 0) &&
        all(grepl("^Total (.+) execution time.*$", unlist(ret))))
        message("done")
    else
    ...
    if ("reads_list2" %in% names(cfg)) {
        ...
        callstr <- paste("P",
                         paste(
                             shQuote(
                                 file.path(path.expand(cfg[['temp_path']]),
                                           c('read_1_1', 'read_1_1.names',
                                             'read_1_1.quals',
                                             'read_1_2', 'read_1_2.names',
                                             'read_1_2.quals', 'unmapped.sam',
                                             sprintf("%s_%s.sam",
                                                     refL[['chrFile']],
                                                     refL[['chrS']])))
                                 ),
                             collapse = " "),
                         sep = " ")
        ret <- system2(file.path(system.file(package = "Rbowtie"), "getSpliceMapUnmapped"),
                       callstr, stdout = TRUE, stderr = FALSE)
    } else {
    ...
    ret <- system2(file.path(system.file(package = "Rbowtie"), "amalgamateSAM"),
                   callstr, stdout = TRUE, stderr = FALSE)
    ...
    ret <- system2(file.path(system.file(package = "Rbowtie"), "fuseReorder"),
                   callstr, stdout = TRUE, stderr = FALSE)
    if (length(ret) > 0 && grepl("^Finished", ret[length(ret)]))
        message("done")
    else
...
.write_cfg <- function(lst, file) {
    ...
    if (length(lst[["reads_list1"]]) != 1)
        stop("'reads_list1' has not exactly one element; ",
             "all reads need to be contained in a single file")
    ...
    if ("reads_list2" %in% names(lst) &&
        length(lst[["reads_list1"]]) != length(lst[["reads_list2"]]))
    ...
    if (!all(file.exists(lst[["bowtie_base_dir"]]))) {
        tmp <- sub(paste(basename(lst[["bowtie_base_dir"]]), "$", sep = ""),
                   "", lst[["bowtie_base_dir"]])
    ...
    if (any(f <- !file.exists(lst[["reads_list1"]])))
        stop(sprintf("non-existing sequence files in 'reads_list1': %s",
                     paste(lst[["reads_list1"]][f], collapse = ", ")))
    ...
    if ("reads_list2" %in% names(lst)) {
        ...
        if(any(f <- !file.exists(lst[["reads_list2"]])))
            stop(sprintf("non-existing sequence files in 'reads_list2': %s",
                         paste(lst[["reads_list2"]][f], collapse = ", ")))
    }
    ...
    if (!all(file.info(lst[["genome_dir"]])$isdir)) {
        ...
        chrs <- normalizePath(list.files(lst[["genome_dir"]],
                                         pattern = "\\.fa$|\\.fna$|\\.fasta$",
                                         full.names = TRUE),
                              winslash = "/")
        if (length(chrs) == 0)
            stop("No chromosome files found in '", lst[["genome_dir"]],
                 "' using pattern '\\.fa$|\\.fna$|\\.fasta$'")
        lst[["genome_files"]] <- paste(chrs, collapse = "\n")
    }

    # check selectSingleHit
    if (!is.logical(lst[["selectSingleHit"]]) ||
        length(lst[["selectSingleHit"]]) != 1L)
        stop("'selectSingleHit' must be TRUE or FALSE")
    ...
    if (!"read_mismatch" %in% names(lst)  ||  is.null(lst[["read_mismatch"]]))
        lst[["read_mismatch"]] <- 2
    if (!"num_chromosome_together" %in% names(lst)  ||
        is.null(lst[["num_chromosome_together"]]))
    ...
    if (any(f <- lengths(lst2) != 1))
        stop(sprintf("only a single value can be given to settings: %s",
                     paste(names(lst2)[f], collapse = ", ")))
    ...
    out <- c("# SpliceMap config file, created by QuasR",
             ...
             sprintf("%s = %s", names(lst2)[-i],
                     unlist(lapply(lst2[-i], as.character))))
